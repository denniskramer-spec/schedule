...
case "${1:-windows}" in
  ...
  windows)
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 \
      go build -trimpath -ldflags="-s -w -H windowsgui -X main.version=${VERSION:-2.6} -X main.defaultUpdateURL=${UPDATE_URL:-https://github.com/denniskramer-spec/schedule/releases/latest/download/latest.json}" -o schedule.exe .
    ls -lh schedule.exe
    ;;
  installer)
    "$0" windows
    ...
    CGO_ENABLED=0 GOOS=windows GOARCH=amd64 \
      go build -trimpath -ldflags="-s -w -H windowsgui -X main.version=${VERSION:-2.6}" \
    ...
    printf '{"version": "%s", "file": "Schedule-Setup.exe", "sha256": "%s", "notes": "%s"}\n' \
      "${VERSION:-2.6}" "$sum" "${NOTES:-}" > latest.json
  ...
  icons)
    ...
    $winres simply --arch amd64,arm64 --manifest gui --icon icon.ico \
      ...
      --product-version "${VERSION:-2.6}" --file-version "${VERSION:-2.6}" \
